...
(function initQuickStrokeDataContract(global) {
  ...
  function completeScreeningSession(completionReason = 'FULL_SCREENING_COMPLETED') {
    ...
    store.setItem('fast_session_completion_reason', String(completionReason));
    return getSessionContext();
  }
  ...
  function closeScreeningSession(status, reasonCode) {
    ...
    store.setItem(SESSION_KEYS.screeningCompletedAt, nowIso());
    if (reasonCode) store.setItem('fast_session_completion_reason', String(reasonCode));
    return getSessionContext();
  }
  ...
  function inspectSourceIntegrity() {
    ...
    const resultConfig = global.QS_CONFIG?.result || {};
    if (resultConfig.policyVersion && resultConfig.policyVersion !== RESULT_POLICY_VERSION) {
      errors.push('RESULT_POLICY_VERSION_MISMATCH');
    }
    if (!resultConfig.policyVersion) warnings.push('RESULT_POLICY_VERSION_NOT_DECLARED_IN_CONFIG');
